...
var _KMZLoader = require('../../../Renderer/ThreeExtended/KMZLoader');
...
function KML_Provider(ellipsoid) {
    ...
    this.kmzLoader = new _KMZLoader2.default();
    this.cache = new _map2.default();
}

KML_Provider.prototype = (0, _create2.default)(_Provider2.default.prototype);
...
KML_Provider.prototype.loadKMZ = function (longitude, latitude) {
    ...
    return this.getUrlCollada(longitude, latitude).then(function (result) {
        ...
        if (result.scene.children[0]) {
            ...
            var position = _this.ellipsoid.cartographicToCartesian(coorCarto);
            coorCarto.altitude = 0;
            var normal = _this.ellipsoid.geodeticSurfaceNormalCartographic(coorCarto);
...
KML_Provider.prototype.parseKML = function (urlFile, longitude, latitude, networkOptions) {
    var _this2 = this;

    var url = 'http://wxs.ign.fr/' + 'va5orxd0pgzvq3jxutqfuy0b' + '/vecteurtuile3d/BATI3D/FXX/';
    return _Fetcher2.default.xml(urlFile, networkOptions).then(function (result) {
        var NetworkLink = [];
        ...
        for (var i = 0; i < NetworkLink.length; i++) {
            ...
            if (latitude < coords[0] && latitude > coords[1] && longitude < coords[2] && longitude > coords[3]) {
                var href = [];
                ...
                if (href[i].toLowerCase().substr(-4) === '.kml') {
                    return _this2.parseKML(href[i], longitude, latitude);
                }
                // Next level : Get the next KMZ actual position's coords
                        ...
                        var p = _this2.cache[url_kmz];
                        if (!p) {
                            p = _this2.kmzLoader.load(url_kmz);
                            _this2.cache[url_kmz] = p;
                        }
                        return p;
                    }
            }
...
KML_Provider.prototype.getUrlCollada = function (longitude, latitude) {
    ...
    return _Fetcher2.default.xml('http://wxs.ign.fr/va5orxd0pgzvq3jxutqfuy0b/vecteurtuile3d/BATI3D/BU.Building.kml').then(function () {
        // get href's node value
        // var kml_0 = result_0.getElementsByTagName("href");
        var url_href_1 = 'http://wxs.ign.fr/' + 'va5orxd0pgzvq3jxutqfuy0b' + '/vecteurtuile3d/BATI3D/FXX/TREE/0/0_000_000.kml';


        return _this3.parseKML(url_href_1, longitude, latitude);
    });
};

exports.default = KML_Provider;
// If France
